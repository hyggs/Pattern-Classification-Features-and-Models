...
for e in range(IT):
    ...
    for f in range(N1):
        pdf = (pd0 * Weights2[f][0] + pd1 * Weights2[f][1] + pd2 * Weights2[f][2] +
               pd3 * Weights2[f][3] + pd4 * Weights2[f][4] + pd5 * Weights2[f][5] +
               pd6 * Weights2[f][6] + pd7 * Weights2[f][7] + pd8 * Weights2[f][8] +
               pd9 * Weights2[f][9]) * Out1[f] * (1 - Out1[f])
        for m in range(M):
            derivatives1[m, f] = pdf * X[i][m]
